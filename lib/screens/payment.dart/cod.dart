...
class _CODState extends State<COD> {
  ...
  double discount = userController.userModel.value.cart.fold(
    ...
    (previousValue, element) {
      ...
      return 
          previousValue +
              (double.parse(element.discount)*element.number);
  
    },
  );
  double carttotal = userController.userModel.value.cart.fold(
    0,
    (previousValue, element) => previousValue + (double.parse(element.price)*element.number),
  );
  @override
                        ...
                        RichText(
                          text: TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(
                                  text:
                                      '₹${double.parse((discount + int.parse(orderController.ordercongig.shippingfee)).toStringAsFixed(2))}',
                                  style: TextStyle(
                                      fontSize: 18.0,
                                      fontWeight: FontWeight.bold,
                                      color: kprimarycolor)),
